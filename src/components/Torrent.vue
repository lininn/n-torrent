...
<div>
    <div class="row">
      <div class="col-md-12">
         <h3>种子转磁力链接</h3>
        <input type="file" id="file" @change="selectFile" name="files"  style="display:none;">
    ...
    <div v-if="torrentInfo" class="row">
        ...
        <div class="input-group">
          ...
          <span class="input-group-btn">
            <button type="button" id="copy-magnet"  class="btn btn-default copy-button" title="复制到剪贴板"
            v-clipboard:copy="torrentInfo.magnet"
        ...
        <ul v-for="(file,index) in torrentInfo.files.files" :key="index+Math.random()">
            <li>{{ file.name }} <span class="grey">( {{ file.size | readablizeBytes }} )</span></li>
        </ul>
        <h3>announce ({{ torrentInfo['announce-list'].length }}) 个</h3>
        <ul v-for="(announce,index) in torrentInfo['announce-list']" :key="index+Math.random()">
            <li>{{ announce }}</li>
        </ul>
        </div>
    </div>
  ...
  methods: {
    ...
    startSelectFiles: function(){
        window.document.getElementById('file').click();
          this.Common.pushToArr(['_trackEvent', 'selectFile', 'start']);
    },
    ...
    //   if (file.type.match('application/x-bittorrent') || file.type.match('application/octet-stream')) {
           ...
           axios.post(config.apiUrl+'api/index.php', formData)
           .then((response) => {
             this.button_status = 'ready';
             this.torrentInfo = response.data;
               this.Common.pushToArr(['_trackEvent', 'selectFile', 'done']);
           }).catch(err => {
             console.log(err);
               this.button_status = 'ready';
                 this.Common.pushToArr(['_trackEvent', 'selectFile', 'error']);
