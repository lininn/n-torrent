    ...
    <div class="col-md-12">
          ...
          <div class="input-group">
            ...
            <span class="input-group-btn">
              ...
              <button  v-if="status=='pending'" class="btn btn-primary" disabled @click="getTorrent" type="button" style="width:72px;">处理中...</button>
                <button v-else class="btn btn-primary" @click="getTorrent" type="button" style="width:72px;">转换</button>
            </span>
          </div>
    </div>
    <div v-if="downloadUrl && status=='done'" class="col-md-12">
        <a :href="downloadUrl" onclick="  this.Common.pushToArr(['_trackEvent', 'convert', 'download']);" download class="btn btn-default" style="margin-top:20px;">点此下载种子</a>
        ...
        <ul v-for="(file,index) in torrentInfo.files.files" :key="index">
            <li>{{ file.name }} <span class="grey">( {{ file.size | readablizeBytes }} )</span></li>
        </ul>
        <h3>announce ({{ torrentInfo['announce-list'].length }}) 个</h3>
        <ul v-for="(announce,index) in torrentInfo['announce-list']" :key="index">
            <li>{{ announce }}</li>
        </ul>
...
export default {
  ...
  methods: {
      getTorrent: function(){
           this.Common.pushToArr(['_trackEvent', 'convert', 'start']);
          if(this.magnet == ""){
              alert('请输入磁力链接');
              return;
          }
         ...
         // this.status = "pending";
          ...
          axios.get(config.apiUrl + 'api/hash2Torrent.php', {
              params: {
                infohash: infohash.toLocaleUpperCase()
              }
          })
          .then((response) => {
             this.status = "done";
              if(response.status==200){
                  ...
                  this.downloadUrl = "/api/download.php?infohash=" + response.data.infohash;
                  window.open(response.data.download_url)
                    this.Common.pushToArr(['_trackEvent', 'convert', 'success']);
              }else{
                  this.status = 'done';
                  this.downloadUrl = "";
                    this.Common.pushToArr(['_trackEvent', 'convert', 'no torrent']);
              }
            }).catch(err => {
                this.status = 'done';
                  this.Common.pushToArr(['_trackEvent', 'convert', 'error']);
                console.log(err);
            });
      }
  },
